...
public abstract class WorldData<T> implements DataProvider
{
    ...
    protected void add(String name, T obj)
    {
        try
        {
            DataProvider.save(GSON, cache, codec.encodeStart(JsonOps.INSTANCE, obj).getOrThrow(false, Wyrmroost.LOG::error), generator.getOutputFolder().resolve("data/wyrmroost/worldgen/" + path + "/" + name + ".json"));
        }
        catch (IOException e)
        {
