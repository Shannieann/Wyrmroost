...
import static com.github.shannieann.wyrmroost.registry.WRBlocks.REGISTRY;
...
class BlockModelData extends BlockStateProvider
{
    ...
    protected void registerStatesAndModels()
    {
        ...
        for (Block block : registered)
        {
            ...
            if (block instanceof CropBlock) crop((CropBlock) block);
            //else if (block instanceof BushBlock || block instanceof GrowingPlantBlock || block instanceof CoralPlantBlock)
                //cross(block);
            //else if (block instanceof PetalsBlock) petals(block);
            else if (block instanceof InfestedBlock)
                simpleBlock(block, models().withExistingParent(block.getRegistryName().getPath(), ((InfestedBlock) block).getHostBlock().getRegistryName()));
            //else if (block instanceof SnowyDirtBlock) snowy(block);
            else /*if (!(block instanceof Fluid))*/ simpleBlock(block);
        }
    }

    void crop(CropBlock block)
    {
    ...
    void layered(Block block, ResourceLocation texture)
    {
        ...
        {
            int value = s.getValue(SnowLayerBlock.LAYERS);
            if (value == 8)
